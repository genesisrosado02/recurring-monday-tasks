...
app.post('/calculate-task-with-tag', async (req, res) => {
    try {
        ...
        // 1. Get the Nth and Day values from the user input
        ...
        // 2. Calculate the specific Date for THIS month
        const now = new Date();
        let d = new Date(now.getFullYear(), now.getMonth(), 1); // Start at the 1st of the current month
        
        // Find the first instance of the requested day (e.g., the first Friday)
        while (d.getDay() !== parseInt(day)) {
            d.setDate(d.getDate() + 1);
        }
        
        // Jump to the Nth instance (e.g., if Nth is 3, jump 2 more weeks)
        d.setDate(d.getDate() + (parseInt(nth) - 1) * 7);

        const dueDateString = d.toISOString().split('T')[0];
        console.log(`📅 Creating task for today (1st), setting Due Date to: ${dueDateString}`);

        // 3. Prepare the mutation
        const columnValues = {
            [process.env.DUE_DATE_COLUMN_ID]: { "date": dueDateString },
        ...
        if (response.data.errors) {
            ...
            console.log(`✅ Success! Task created with Due Date: ${dueDateString}`);
        }
        res.status(200).send({});
    } catch (err) {
